...
void macro()
{
    ...
    double _mmin = mmin;  double _mmax = mmax;
    ...
    // I WASN'T ABLE TO USE THE SAME MODEL ON BOTH HISTOGRAMS SO I HAD TO DUPLICATE THIS CODE TWICE------------------------
        ...
        //RooDataSet dh("dh","dh",mass_all,RooFit::Import(*hist_all));
    ...
    frame_new->Draw("");
       ...
       //   - REQUESTED CHANGE -   -------------------------------
        return;
        RooFitResult* fitres = model->fitTo(dh, RooFit::Save());
        
        cout << "\n\n\n\n\n";
        fitres->Print();
        cout << "\n\n\n\n\n";
    
    // CREATES string ARRAY WITH EFFICIENCY QUANTITIES
        string* quantities_all = new string[9];
    
        quantities_all[0] = "frac";
        quantities_all[1] = "frac";
        quantities_all[2] = "lambda";
        quantities_all[3] = "mean1";
        quantities_all[4] = "mean2";
        quantities_all[5] = "mean3";
        quantities_all[6] = "n_back";
        quantities_all[7] = "n_signal_total";
        quantities_all[8] = "sigma";

        // CREATES char** TO ACCESS QUANTITIES FIT VALUES -- find() doesn't accept string input
        char **ALL_efficiencies = new char*[9];
        
        RooRealVar** ALL = new RooRealVar*[9];
        
        for (int i = 0; i < 9; i++)
        {
            char* save = new char[quantities_all[i].length()];
            strcpy(save, quantities_all[i].c_str());
            ALL[i] = (RooRealVar*)fitres->floatParsFinal().find(save);
            delete[] save;
        }
    
        delete[] ALL_efficiencies;
        
        double n_ALL = ALL[7]->getVal();
        double n_ALL_err  = ALL[7]->getError();
        //----------------------------------------------------------------------------
...
// HERE IS THE REPLICATE----------------------------------------------------------------------------------------------------
    ...
    RooRealVar n_back_pass("n_back_pass","n_back_pass",n_back_initial_pass, 0.,dh_pass.sumEntries());
        ...
        RooFitResult* fitres_pass = model_pass->fitTo(dh_pass, RooFit::Save());
        ...
        quantities[0] = "frac_pass1";
        quantities[1] = "frac_pass2";
        quantities[2] = "lambda_pass";
        quantities[3] = "mean1_pass";
        quantities[4] = "mean2_pass";
        quantities[5] = "mean3_pass";
        quantities[6] = "n_back_pass";
        quantities[7] = "n_signal_total_pass";
        quantities[8] = "sigma_pass";

        // CREATES char** TO ACCESS QUANTITIES FIT VALUES -- find() doesn't accept string input
        char **PASS_efficiencies = new char*[9];
        
        RooRealVar** PASS = new RooRealVar*[9];
        
        for (int i = 0; i < 9; i++)
        {
            char* save = new char[quantities[i].length()];
            strcpy(save, quantities[i].c_str());
            PASS[i] = (RooRealVar*)fitres_pass->floatParsFinal().find(save);
            delete[] save;
        }
    
        delete[] PASS_efficiencies;
        //PASS[0] = (RooRealVar*)fitres_pass->floatParsFinal().find(PASS_efficiencies[3]);
        
        double n_PASS      = PASS[7]->getVal();
        double n_PASS_err  = PASS[7]->getError();
        //----------------------------------------------------------------------------
    ...
    //for (int i=0; i < 9; i++)
    //    cout << "CALCULATED EFFICIENCY VALUE FOR IS: " << PASS[i]->getVal()/ALL[i]->getVal() << endl;
    cout << "CALCULATED EFFICIENCY IS " << n_PASS/n_ALL << endl;
    delete[] quantities_all;
}
